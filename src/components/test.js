...
const Spec: React.FC = () => {
  ...
  return (
    <div className="container">{specList.map(({ title, list }, index) => (<div key={index}><p className="title">{title}p><div className="specBox">{list.map((value, i) => {
      ...
      return (<span
        ...
        onClick={() => handleClick(isOption, value, index)}>{value}span>);})}div>div>))}div>);};
                  export default Spec;